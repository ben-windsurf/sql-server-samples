...
WHERE d.[Date] >= DATEADD(QUARTER, -4, GETDATE())
GROUP BY 
    d.[Calendar Year],
    d.[Calendar Quarter Number],
    d.[Calendar Quarter Label]
ORDER BY d.[Calendar Year], d.[Calendar Quarter Number];

GO
EXECUTE sp_addextendedproperty @name = N'Description', @value = N'Quarterly revenue and margin trends for the last 4 quarters from WideWorldImportersDW', @level0type = N'SCHEMA', @level0name = N'Reports', @level1type = N'VIEW', @level1name = N'QuarterlyTrends';
